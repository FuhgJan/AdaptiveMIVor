classdef OK_model
    ...
    methods
        ...
        function x_new = adaptive_sampling(obj,method,A,class_Limit)
            ...
            function newPoint =doVarianceForPoint(obj,points)
%                      disp('Variance')
                    beta_hat = compute_beta_hat(obj,obj.R);
                    sigma_sq_hat = compute_sigma_sq_hat(obj,obj.R,beta_hat);
                    %                 Distance = inf;
            ...
            function [newPoint,points] = findNewPointInHighestVolume(obj,C, indexOfInterest,class_Limit)
...
%                 closestPoints_class2Sum = 0;
                for kk=1:numel(indexNeighbor)
                    if Y_without_indexOfInterest(indexNeighbor(kk)) < class_Limit
                ...
                if closestPoints_class2
                    ...
                    for ll=1:size(points,1)
                        ...
                        if d< Distance
                           ...
                           newPointTemp =doVarianceForPoint(obj,points);
                end
                newPoint = newPointTemp;
...
%                addpath('help_functions')
            ...
            function x_new = MiVor_function(obj,A, class_Limit)
               % addpath('help_functions')
                ...
                scatter(obj.X(IndexClass1,1),obj.X(IndexClass1,2),80,'b','filled'); hold on;
                scatter(obj.X(IndexClass2,1),obj.X(IndexClass2,2),80,'y','filled'); hold on;
                scatter(-1,-1,100,'k','filled'); hold off;
                lgd = legend({'Voronoi points','Voronoi cells','Sample belonging to Class 1', 'Sample belonging to Class 2', 'New found point'}, 'Location','southoutside');
                lgd.NumColumns = 2;                xlabel('$x_{1}$','Interpreter','Latex')
                ...
                if isempty(IndexClass1) || (r<exploration)
                    ...
                    [newPoint,points] = findNewPointInHighestVolume(obj,voronoi_matrix,indexmaxVol,class_Limit);
                    [~,D] = knnsearch(obj.X,newPoint,'K',obj.m);
                    
                    S = 0.35*SFVCT_S(obj);
                    
                    while numel(find(D< S))>1
%                         disp('Too Close')
                        [~,D] = knnsearch(obj.X,newPoint,'K',obj.m);
                        rr = rand();
                        if rr>0.2
                            newPoint =doVarianceForPoint(obj,points);
                        else
                        newPoint = DoMIPT(obj,A);
                        end
                    end
                ...
                scatter(obj.X(IndexClass1,1),obj.X(IndexClass1,2),80,'b','filled'); hold on;
                scatter(obj.X(IndexClass2,1),obj.X(IndexClass2,2),80,'y','filled'); hold on;
                scatter(x_new(:,1),x_new(:,2),100,'k','filled'); hold off;
                lgd = legend({'Voronoi points','Voronoi cells','Sample belonging to Class 1', 'Sample belonging to Class 2', 'New found point'}, 'Location','southoutside');
                lgd.NumColumns = 2;
                xlim([0 1])
                ylim([0 1])
